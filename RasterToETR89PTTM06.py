...
class RasterToETR89PTTM06(GdalAlgorithm):

    INPUT = 'INPUT'
    OUTPUT = 'OUTPUT'
    INPUT_CRS = 'CRS'
    INPUT_CRS_OPTIONS = ['Datum Lisboa [EPSG:20791 / EPSG:5018 / ESRI:102165]',
    ...
    def defineCharacteristics(self):
        self.name = 'Rasters para PT-TM06/ETRS89'
        self.group = 'Transformação de Datum em Rasters'
        self.addParameter(ParameterRaster(self.INPUT, 'Ficheiro de Entrada', False))
        self.addParameter(ParameterSelection(self.INPUT_CRS, 'CRS da Layer Original',
                          self.INPUT_CRS_OPTIONS))
    ...
    def processAlgorithm(self, progress):
        arguments = ['-s_srs']
        if self.getParameterValue(self.INPUT_CRS) == 0:
            # Datum Lisboa
        ...
        elif self.getParameterValue(self.INPUT_CRS) == 1:
        ...
        elif self.getParameterValue(self.INPUT_CRS) == 2:
            # Datum 73
            if self.getParameterValue(self.GRID) == 0:
                # Jose Alberto Goncalves
        ...
        elif self.getParameterValue(self.INPUT_CRS) == 3:
            # Datum 73 Militar
            if self.getParameterValue(self.GRID) == 0:
                # Jose Alberto Goncalves
