...
package: clean all
	cd .. && rm -f *.zip && zip -r processing_pttransform.zip processing_pttransform -x \*.pyc \*.ts \*.ui \*.qrc \*.pro \*~ \*.git\* \*Makefile*
	mv ../processing_pttransform.zip .

upload: package
	plugin_uploader.py processing_pttransform.zip
